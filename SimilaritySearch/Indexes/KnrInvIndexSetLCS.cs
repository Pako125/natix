...
//
//   Original filename: natix/natix/SimilaritySearch/Indexes/KnrInvIndexSetLCS.cs
// 
...
namespace natix.SimilaritySearch
{
	...
	/// </summary>
	public class KnrInvIndexSetLCS<T> : KnrInvIndexBase<T>
	{
		...
		public override void FinalizeLoad (string name, IDictionary<string, object> config)
		{
			...
			this.MaxDist = 2 * (Math.Abs (this.KnrBound) - 1);
		}
		...
		public override void Configure (IEnumerable<string> args)
		{
			...
			OptionSet op = new OptionSet () {
				{"maxdist", "Maximum LCS distance to considere candidates", v => this.MaxDist = int.Parse (v)},
			};
			op.Parse (args);
		}
		...
		protected override IResult GetCandidates (T q, IList<ushort> qseq, int k)
		{
			...
			foreach (var docid in C) {
				...
				var dist = SequenceSpace<ushort>.LCS (qseq, useq);
